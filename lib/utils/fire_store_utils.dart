...
import 'package:goride/constant/collection_name.dart';
import 'package:goride/constant/constant.dart';
import 'package:goride/constant/show_toast_dialog.dart';
import 'package:goride/model/admin_commission.dart';
import 'package:goride/model/airport_model.dart';
import 'package:goride/model/banner_model.dart';
import 'package:goride/model/conversation_model.dart';
import 'package:goride/model/coupon_model.dart';
import 'package:goride/model/currency_model.dart';
import 'package:goride/model/driver_user_model.dart';
import 'package:goride/model/bank_details_model.dart';
import 'package:goride/model/document_model.dart';
import 'package:goride/model/driver_document_model.dart';
import 'package:goride/model/driver_rules_model.dart';
import 'package:goride/model/vehicle_type_model.dart';
import 'package:goride/model/withdraw_model.dart';
import 'package:goride/model/faq_model.dart';
import 'package:goride/model/freight_vehicle.dart';
import 'package:goride/model/inbox_model.dart';
import 'package:goride/model/intercity_order_model.dart';
import 'package:goride/model/intercity_service_model.dart';
import 'package:goride/model/language_model.dart';
import 'package:goride/model/language_privacy_policy.dart';
import 'package:goride/model/language_terms_condition.dart';
import 'package:goride/model/on_boarding_model.dart';
import 'package:goride/model/order/driverId_accept_reject.dart';
...
import 'package:goride/model/service_model.dart';
import 'package:goride/model/sos_model.dart';
...
import 'package:goride/model/zone_model.dart';
import 'package:goride/widget/geoflutterfire/src/geoflutterfire.dart';
import 'package:goride/widget/geoflutterfire/src/models/point.dart';
...
class FireStoreUtils {
  ...
  static Future<bool> isDriverLogin() async {
    bool isLogin = false;
    if (FirebaseAuth.instance.currentUser != null) {
      isLogin = await driverExitOrNot(FirebaseAuth.instance.currentUser!.uid);
    } else {
      isLogin = false;
    }
    return isLogin;
  ...
  static Future<bool> driverExitOrNot(String uid) async {
    bool isExit = false;
    await fireStore.collection(CollectionName.driverUsers).doc(uid).get().then(
      ...
      log("Failed to check driver: $error");
      isExit = false;
    });
    return isExit;
  }
  ...
  static Future<DriverUserModel?> getDriverProfile(String uuid) async {
    ...
    await fireStore.collection(CollectionName.driverUsers).doc(uuid).get().then((value) {
      ...
      log("Failed to get driver: $error");
      driverUserModel = null;
    });
    return driverUserModel;
  }

  static Future<bool> updateDriverUser(DriverUserModel driverModel) async {
    bool isUpdate = false;
    await fireStore.collection(CollectionName.driverUsers).doc(driverModel.id).set(driverModel.toJson()).whenComplete(() {
      isUpdate = true;
    }).catchError((error) {
      log("Failed to update driver: $error");
      isUpdate = false;
    });
    return isUpdate;
  }
